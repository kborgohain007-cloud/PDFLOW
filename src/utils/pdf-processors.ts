...
// 7. PDF to Word (Server-Side using PaddleOCR PP-Structure)
export async function processPdfToWord(
  ...
  onProgress(10, 'Establishing secure connection to server...');
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData?.detail || 'Server processing failed. Please try again.');
    }

    onProgress(90, 'Downloading converted document...');
    const blob = await response.blob();
    
    const baseName = file.name.substring(0, file.name.lastIndexOf('.')) || file.name;
    return { blob, fileName: `${baseName}_converted.docx` };
  } catch (error: any) {
